...
class TestMRFGeneration_paletted(unittest.TestCase):
    ...
    def test_generate_mrf(self):
        ...
        self.assertEqual(str(dataset.GetProjection()),'GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,AUTHORITY["EPSG","7030"]],AUTHORITY["EPSG","6326"]],PRIMEM["Greenwich",0,AUTHORITY["EPSG","8901"]],UNIT["degree",0.0174532925199433,AUTHORITY["EPSG","9122"]],AXIS["Latitude",NORTH],AXIS["Longitude",EAST],AUTHORITY["EPSG","4326"]]')
...
class TestMRFGeneration_brunsli_off(unittest.TestCase):
...
class TestMRFGeneration_zenjpeg_brunsli(unittest.TestCase):

    def setUp(self):
        testdata_path = os.path.join(os.getcwd(), 'mrfgen_files')
        self.staging_area = os.path.join(os.getcwd(), 'mrfgen_test_data')
        self.tmp_area = os.path.join(self.staging_area, 'tmp')
        test_config = os.path.join(testdata_path, "mrfgen_test_config19.xml")
        ...
        self.output_mrf = os.path.join(self.staging_area, "output_dir/GOES-East_B13_LL_v0_NRT_ZENJPEG_BRUNSLI2021100000000.mrf")
        self.output_pjg = os.path.join(self.staging_area, "output_dir/GOES-East_B13_LL_v0_NRT_ZENJPEG_BRUNSLI2021100000000.pjg")
        self.output_idx = os.path.join(self.staging_area, "output_dir/GOES-East_B13_LL_v0_NRT_ZENJPEG_BRUNSLI2021100000000.idx")
        self.output_img_png = os.path.join(self.staging_area, "output_dir/GOES-East_B13_LL_v0_NRT_ZENJPEG_BRUNSLI2021100000000.png")
        self.output_img_jpg = os.path.join(self.staging_area, "output_dir/GOES-East_B13_LL_v0_NRT_ZENJPEG_BRUNSLI2021100000000.jpg")
        self.compare_img_jpg = os.path.join(testdata_path, "test_comp19.jpg")
...
class TestMRFGeneration_background(unittest.TestCase):

    def setUp(self):
        testdata_path = os.path.join(os.getcwd(), 'mrfgen_files')
        self.staging_area = os.path.join(os.getcwd(), 'mrfgen_test_data')
        self.tmp_area = os.path.join(self.staging_area, 'tmp')
        test_config = os.path.join(testdata_path, "mrfgen_test_config17.xml")
        ...
        self.output_mrf = os.path.join(self.staging_area, "output_dir/OCI_PACE_True_Color2024152_.mrf")
        self.output_pjg = os.path.join(self.staging_area, "output_dir/OCI_PACE_True_Color2024152_.pjg")
        self.output_idx = os.path.join(self.staging_area, "output_dir/OCI_PACE_True_Color2024152_.idx")
        self.output_img_jpg = os.path.join(self.staging_area, "output_dir/OCI_PACE_True_Color2024152_.jpg")
        self.compare_img_jpg = os.path.join(testdata_path, "test_comp17.jpg")

        # generate MRF
        cmd = "mrfgen -c " + test_config
        run_command(cmd, show_output=DEBUG)
        run_command('mrf_read.py --input ' + self.output_mrf + ' --output ' + self.output_img_jpg + ' --tilematrix 2 --tilecol 0 --tilerow 0', show_output=DEBUG)
    ...
    def test_generate_mrf(self):
        ...
        if DEBUG:
            print('Size: ',dataset.RasterXSize,'x',dataset.RasterYSize, 'x',dataset.RasterCount)
        self.assertEqual(dataset.RasterXSize, 2560, "Size does not match")
        self.assertEqual(dataset.RasterYSize, 1280, "Size does not match")
        self.assertEqual(dataset.RasterCount, 3, "Not a paletted image")
...
if __name__ == '__main__':
    ...
    available_tests = {
        ...
        'brunsli_on': TestMRFGeneration_brunsli_on,
        'brunsli_off': TestMRFGeneration_brunsli_off,
        'zenjpeg_brunsli': TestMRFGeneration_zenjpeg_brunsli,
